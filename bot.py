...
# Создаём таблицы при первом импорте модуля — работает и в режиме polling
# (запуск через `python bot.py`), и в режиме вебхука (импорт из flask_app.py).
db.init_db()
...
# Раньше их по расписанию запускал apscheduler внутри процесса — на бесплатном
# PythonAnywhere постоянно работающий процесс держать нельзя, поэтому теперь
# эти же функции дёргаются снаружи: по расписанию в GitHub Actions, через
# защищённые секретом HTTP-эндпоинты в flask_app.py.
# ============================================================================
...
# ============================================================================
# Обработка вебхука (используется flask_app.py на PythonAnywhere).
# Bot и Dispatcher создаются один раз при импорте модуля (см. выше), а тут —
# только разбор входящего апдейта и передача его в диспетчер aiogram.
# После обработки закрываем HTTP-сессию бота: так как каждый запрос к
# Flask-приложению обслуживается в своём event loop (через asyncio.run),
# сессию нужно пересоздавать заново на каждый запрос, иначе будет ошибка
# "Event loop is closed" на втором сообщении подряд.
# ============================================================================
...
# ============================================================================
# Запуск в режиме polling — только для локальной разработки/тестирования.
# В проде на PythonAnywhere бот работает через вебхук (см. flask_app.py),
# эта функция там не используется.
# ============================================================================
async def main():
    if not config.BOT_TOKEN:
        raise RuntimeError("Не задан BOT_TOKEN в переменных окружения!")

    log.info("Бот запущен в режиме polling (локальная разработка).")
